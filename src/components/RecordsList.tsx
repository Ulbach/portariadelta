...
const RecordsList: React.FC<RecordsListProps> = ({ records, loading, onRefresh, onBack }) => {
  ...
  const handleClear = async () => {
    if (window.confirm("Tem certeza que deseja apagar TODO o histórico de registros no Firebase? Esta ação não pode ser desfeita.")) {
      setIsClearing(true);
      const success = await firebaseService.clearAllRecords();
      if (success) {
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
          <div className="flex flex-col">
            ...
            <span className="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">Histórico Cloud</span>
          </div>
        ...
        <div className="divide-y divide-slate-50">
          {loading ? (
            ...
            <div className="p-16 text-center text-slate-400 text-sm italic">
              Nenhum registro encontrado no Firebase.
            </div>
          )}
        </div>
      </div>
