...
const App: React.FC = () => {
  ...
  const handleRegisterAction = async (name: string, type: 'ENTRY' | 'EXIT') => {
    setNotifying('🚀 Sincronizando...');
    const success = await sheetService.appendRecord({ name, company: partners.find(p => p.name === name)?.company || 'Parceiro' }, type);
    if (success) {
      setNotifying('✅ Sucesso!');
      setTimeout(() => { setNotifying(null); loadData(true); }, 2000);
      return true;
    }
    setNotifying('❌ Erro.'); setTimeout(() => setNotifying(null), 3000);
    return false;
  };
  ...
  return (
    <div className="min-h-screen flex flex-col max-w-lg mx-auto bg-slate-50 shadow-2xl relative overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-4 no-scrollbar pb-40">
        ...
        {view === ViewMode.REGISTRATION && <Registration partners={partners} activeReports={activeNow} onQuickRegister={handleRegisterAction} initialTab={registrationInitialTab} onBack={() => setView(ViewMode.DASHBOARD)} />}
      ...
      <footer className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-lg p-4 z-50">
        ...
        {view !== ViewMode.WELCOME && (
          <nav className="bg-white/95 backdrop-blur-xl border border-slate-100 shadow-2xl rounded-[32px] p-2 flex justify-between items-center px-4">
            <button onClick={() => setView(ViewMode.WELCOME)} className="text-slate-400 p-2 flex flex-col items-center flex-1">
              <span className="w-5 h-5 mb-1">{ICONS.HOME}</span>
              <span className="text-[8px] font-black uppercase">Início</span>
            </button>
            
            <button onClick={() => setView(ViewMode.DASHBOARD)} className={`flex-1 py-3 rounded-2xl font-black text-[10px] uppercase tracking-wider mx-1 ${view === ViewMode.DASHBOARD ? 'bg-[#5b806d] text-white shadow-lg' : 'text-slate-400'}`}>Painel</button>

            <button onClick={() => setView(ViewMode.PARTNERS)} className={`flex-1 py-3 rounded-2xl font-black text-[10px] uppercase tracking-wider mx-1 ${view === ViewMode.PARTNERS ? 'bg-[#5b806d] text-white shadow-lg' : 'text-slate-400'}`}>Empresas</button>
          </nav>
        )}
      </footer>
    </div>
